...
@interface SJAppPicker () <UISearchResultsUpdating, UISearchBarDelegate>
...
+ (NSArray<SJInstalledApp *> *)enumerateApps
{
	...
	if (!cls) return out;
	id ws = [(id)cls performSelector:NSSelectorFromString(@"defaultWorkspace")];
	if (!ws) return out;

	NSArray *all = [ws performSelector:NSSelectorFromString(@"allInstalledApplications")];
	if (![all isKindOfClass:[NSArray class]]) return out;

	for (id proxy in all) {
		if (![proxy respondsToSelector:NSSelectorFromString(@"bundleIdentifier")]) continue;
           ...
           onFinish:(void (^)(NSArray<NSString *> *))onFinish
{
	...
	vc.multi = multi;
	vc.selected = [[NSMutableOrderedSet orderedSetWithArray:pre.allObjects] mutableCopy];
	vc.onFinish = onFinish;
	vc.allApps = [self enumerateApps];
	vc.visibleApps = vc.allApps;
...
- (void)updateSearchResultsForSearchController:(UISearchController *)sc
{
	...
	NSString *kw = [q stringByTrimmingCharactersInSet:
...
- (void)finishTapped
{
	NSArray<NSString *> *out = [self.selected copy];
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip
{
	...
	cell.imageView.image = SJPAppIcon(a.bundleID); // 取不到就留空，不阻塞列表
